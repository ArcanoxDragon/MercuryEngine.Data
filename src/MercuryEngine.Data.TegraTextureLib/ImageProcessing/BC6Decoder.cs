...
internal static class BC6Decoder
{
	...
	{
		ReadOnlySpan<EncodingBlock> blocks = MemoryMarshal.Cast<byte, EncodingBlock>(data);
	...
	private static void DecodeBlock(EncodingBlock block, Span<ulong> output, int w, int h, int width, bool signed)
	{
	...
	private static int DecodeEndPoints(ref EncodingBlock block, Span<RgbaColor32> endPoints, int mode, bool signed)
	{
		ulong low = block.Low;
