...
public static class BCnDecoder
{
				...
				for (int z = 0; z < depth; z++)
				{
					for (int y = 0; y < h; y++)
					{
						...
						for (int x = 0; x < w; x++)
						{
							...
							BCnDecoder.BC1DecodeTileRgb(tile, data);
				...
				for (int z = 0; z < depth; z++)
				{
					for (int y = 0; y < h; y++)
					{
						...
						for (int x = 0; x < w; x++)
						{
							...
							BCnDecoder.BC23DecodeTileRgb(tile, data[8..]);
				...
				for (int z = 0; z < depth; z++)
				{
					for (int y = 0; y < h; y++)
					{
						...
						for (int x = 0; x < w; x++)
						{
							...
							BCnDecoder.BCnLerpAlphaUnorm(rPal);
							BCnDecoder.BCnDecodeTileAlphaRgba(tile, rPal, block >> 16);
				...
				for (int z = 0; z < depth; z++)
				{
					for (int y = 0; y < h; y++)
					{
						...
						for (int x = 0; x < w; x++)
						{
							...
							if (signed)
							{
								BCnDecoder.BCnLerpAlphaSnorm(rPal);
							}
							else
							{
								BCnDecoder.BCnLerpAlphaUnorm(rPal);
							...
							BCnDecoder.BCnDecodeTileAlpha(tile, rPal, block >> 16);
				...
				for (int z = 0; z < depth; z++)
				{
					for (int y = 0; y < h; y++)
					{
						...
						for (int x = 0; x < w; x++)
						{
							...
							{
								BCnDecoder.BCnLerpAlphaSnorm(rPal);
								BCnDecoder.BCnLerpAlphaSnorm(gPal);
							}
							else
							{
								BCnDecoder.BCnLerpAlphaUnorm(rPal);
								BCnDecoder.BCnLerpAlphaUnorm(gPal);
							}

							BCnDecoder.BCnDecodeTileAlpha(rTile, rPal, blockL >> 16);
							BCnDecoder.BCnDecodeTileAlpha(gTile, gPal, blockH >> 16);

							if (( copyWidth | copyHeight ) == 4)
							{
								outputLine0[x] = BCnDecoder.InterleaveBytes(rTileAsUint[0], gTileAsUint[0]);
								outputLine1[x] = BCnDecoder.InterleaveBytes(rTileAsUint[1], gTileAsUint[1]);
								outputLine2[x] = BCnDecoder.InterleaveBytes(rTileAsUint[2], gTileAsUint[2]);
								outputLine3[x] = BCnDecoder.InterleaveBytes(rTileAsUint[3], gTileAsUint[3]);
							}
							else
							{
								int pixelBaseOOffs = lineBaseOOffs + baseX;
	...
	private static unsafe void BCnDecodeTileAlpha(Span<byte> output, Span<byte> rPal, ulong rI)
	{
		if (Avx2.IsSupported)
		{
	...
	private static unsafe void BCnDecodeTileAlphaRgba(Span<byte> output, Span<byte> rPal, ulong rI)
	{
		if (Avx2.IsSupported)
		{
	...
	private static unsafe void BC1DecodeTileRgb(Span<byte> output, ReadOnlySpan<byte> input)
	{
		...
		clut[0] = BCnDecoder.ConvertRgb565ToRgb888(c0) | 0xff000000;
		clut[1] = BCnDecoder.ConvertRgb565ToRgb888(c1) | 0xff000000;
		clut[2] = BCnDecoder.BC1LerpRgb2(clut[0], clut[1], c0, c1);
		clut[3] = BCnDecoder.BC1LerpRgb3(clut[0], clut[1], c0, c1);

		BCnDecoder.BCnDecodeTileRgb(clut, output, input);
	}

	private static unsafe void BC23DecodeTileRgb(Span<byte> output, ReadOnlySpan<byte> input)
	{
		Span<uint> clut = stackalloc uint[4];
		...
		clut[0] = BCnDecoder.ConvertRgb565ToRgb888(c0);
		clut[1] = BCnDecoder.ConvertRgb565ToRgb888(c1);
		clut[2] = BCnDecoder.BC23LerpRgb2(clut[0], clut[1]);
		clut[3] = BCnDecoder.BC23LerpRgb3(clut[0], clut[1]);

		BCnDecoder.BCnDecodeTileRgb(clut, output, input);
	}

	private static unsafe void BCnDecodeTileRgb(Span<uint> clut, Span<byte> output, ReadOnlySpan<byte> input)
	{
		if (Avx2.IsSupported)
		{
			Span<Vector256<uint>> outputAsVector256 = MemoryMarshal.Cast<byte, Vector256<uint>>(output);
	...
	private static uint BC1LerpRgb2(uint color0, uint color1, uint c0, uint c1)
	{
		if (c0 > c1)
		{
			return BCnDecoder.BC23LerpRgb2(color0, color1) | 0xff000000;
		}
	...
	private static uint BC1LerpRgb3(uint color0, uint color1, uint c0, uint c1)
	{
		if (c0 > c1)
		{
			return BCnDecoder.BC23LerpRgb3(color0, color1) | 0xff000000;
		}
