...
namespace MercuryEngine.Data.TegraTextureLib.Utility
{
	...
	public sealed class MemoryOwner<T> : IMemoryOwner<T>
	{
		...
		{
			this._length = length;
			this._array = ArrayPool<T>.Shared.Rent(length);
		}
		...
		public Memory<T> Memory
		{
			...
			get
			{
				...
				if (array is null)
				{
					MemoryOwner<T>.ThrowObjectDisposedException();
				}

				return new(array, 0, this._length);
			}
		}
		...
		public Span<T> Span
		{
			...
			get
			{
				...
				return MemoryMarshal.CreateSpan(ref firstElementRef, this._length);
			}
		}
		...
		public void Dispose()
		{
			T[]? array = Interlocked.Exchange(ref this._array, null);
